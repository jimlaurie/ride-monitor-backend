...
class SimplifiedDiningScraper {
  ...
  async scrapeDiningData() {
    ...
    try {
      ...
      if (!diningData || diningData.length === 0) {
        console.log('⚠️  No dining data captured, loading from cache...');
  ...
  organizeRestaurants(restaurants) {
    ...
    restaurants.forEach(restaurant => {
      const data = {
        ...
        menuLink: this.buildMenuLink(restaurant),
  ...
  buildMenuLink(restaurant) {
    if (restaurant.webLinks?.dlrDetails?.href) {
      const href = restaurant.webLinks.dlrDetails.href;
  ...
  async loadFromFile() {
    try {
      ...
      console.log('📂 No cache file found, will return empty data');
      return { disneyland: {}, californiaAdventure: {} };
    }
  }
...
module.exports = new SimplifiedDiningScraper();
